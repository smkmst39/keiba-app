 ...
 *
 * Phase 2E Stage 3 (ランダムサンプリング500通り、930R、前後半差3.9pt) で
 * 導出した最適配分を採用。重み付き回収率 88.82% → 93.05% (+4.23pt)。
 * 合計 0.244+0.125+0.198+0.146+0.158+0.071+0.058 = 1.000 */
const WEIGHTS = {
  lastThreeF:   0.244,  // 上がり3F (旧 0.22 → 0.245 を正規化)
  training:     0.125,  // 調教ラスト1F (旧 0.18 → 0.125 大幅減: 近似値で精度低)
  courseRecord: 0.198,  // 同コース成績 (旧 0.18)
  prevClass:    0.146,  // 前走クラス (旧 0.13)
  breeding:     0.158,  // 血統適性 (旧 0.12 → 0.158 大幅増: 実データで効果大)
  weightChange: 0.071,  // 馬体重増減 (旧 0.09)
  jockey:       0.058,  // 騎手評価 (旧 0.08)
} as const;
