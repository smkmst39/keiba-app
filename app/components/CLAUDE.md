...
`RaceReport.tsx` Section 5A で表示する「バックテスト検証済み」3段階推奨 UI。
930R 参加条件グリッドサーチ (4410 条件 × 7 券種) で導出した戦略を、
回収率水準で 3 階層に分けて表示する。

### 推奨レベル

| レベル | 色テーマ | 対応券種 | 条件 | 回収率 |
|---|---|---|---|---|
| ⭐ 本命級 | amber (#b45309) | 馬連 | 両馬 EV≥1.00 かつ スコア≥65 | 172.1% |
| ⭐ 本命級 | amber (#b45309) | 馬単 | 両馬 EV≥1.00 かつ スコア≥65 かつ オッズ≤15 | 119.2% |
| 🎯 堅実級 | blue (#0369a1)  | ワイド | 両馬 EV≥1.02 かつ スコア≥65 かつ オッズ≤10 | 108.3% |
| 📊 参考  | slate (#475569) | 単勝 | EV≥1.00 の最上位 | 80.6% |
| 📊 参考  | slate (#475569) | 複勝 | EV≥1.07 該当馬 | 82.9% |

### 実装ポイント
- 推奨判定は `lib/score/calculator.ts` の
  `shouldRecommendUmaren / shouldRecommendUmatan / shouldRecommendWide /
   shouldRecommendTan / shouldRecommendFuku` に委譲
- 条件を満たさないレースは `SkipCard` で「見送り」理由付き表示
- `EXPECTED_ROI` 定数でバックテスト実績を UI に反映

### 除外している券種
- 三連複・三連単: グリッドサーチで高 ROI 候補はあったが前後半差 >100pt で過学習確定
- サンプル 1500R+ 確保後に再検証する旨を UI 下部に注記
